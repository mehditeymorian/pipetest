...
|---|---|---|---|---|
| 1. Flow structure (prelude lets, exactly one chain line, postlude assertions) | Flow block shape is `let*` then exactly one chain line then assertion lines only. Non-`let` before chain and non-assert after chain are rejected. Missing chain lines are rejected; single-step chains and multi-step arrow chains are both valid. | `FlowDecl`, `FlowPreludeLine`, `FlowChainLine`, `FlowAssertLine` | **parse** (shape/ordering), plus **semantic pass 5** (final hard validation) | `E_FLOW_PRELUDE_NON_LET`, `E_FLOW_MISSING_CHAIN`, `E_FLOW_POSTLUDE_NON_ASSERT` |
| 2. Chain requirements (single-step or arrow chain, alias uniqueness) | Chain may be a single request step or an arrow chain; flow binding names (alias or request name fallback) must be unique within a flow. | `FlowChainLine`, `FlowStepRef` | **semantic pass 5** | `E_DUPLICATE_FLOW_BINDING` |
